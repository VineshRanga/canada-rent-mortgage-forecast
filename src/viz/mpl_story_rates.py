...
def plot_yoy_vs_bank_rate(
    ...
    ax1.set_ylabel('Rent YoY change (%) — captures inflation-like dynamics', color=color1)
    line1 = ax1.plot(plot_df['quarter_label'], plot_df['yoy_pct'], 
                     marker='o', markersize=4, linewidth=1.5, label='Rent YoY %', color=color1)
    ...
    ax2 = ax1.twinx()
    color2 = 'tab:red'
    ax2.set_ylabel('Bank rate (%) — monetary policy signal', color=color2)
    line2 = ax2.plot(plot_df['quarter_label'], plot_df['rate_bank'],
                     marker='s', markersize=4, linewidth=1.5, label='Bank rate', color=color2, linestyle='--')
...
def plot_qoq_vs_goc5y(
    ...
    color1 = 'tab:blue'
    ax1.set_xlabel('Quarter (period end) — aligned quarterly reporting')
    ax1.set_ylabel('Rent QoQ change (%) — short-horizon momentum', color=color1)
    line1 = ax1.plot(plot_df['quarter_label'], plot_df['qoq_pct'],
                     marker='o', markersize=4, linewidth=1.5, label='Rent QoQ %', color=color1)
    ax1.tick_params(axis='y', labelcolor=color1)
    ax1.grid(True, alpha=0.3)
    ...
    ax2 = ax1.twinx()
    color2 = 'tab:red'
    ax2.set_ylabel('5-year GoC yield (%) — medium-term rate benchmark', color=color2)
    line2 = ax2.plot(plot_df['quarter_label'], plot_df['rate_goc_5y'],
                     marker='s', markersize=4, linewidth=1.5, label='GoC 5y yield', color=color2, linestyle='--')
    ax2.tick_params(axis='y', labelcolor=color2)
